...
Репозиторий — монорепо скиллов. `manager` — тонкий оркестратор; переиспользуемые куски вынесены в отдельные
автономные скиллы, которые он вызывает через Skill tool (progressive disclosure — тело `manager/SKILL.md`
держится под 500 строк).

```
skills/
├── manager/                 # оркестратор
│   ├── SKILL.md             # поведенческое ядро + навигация
│   └── references/
│       ├── dispatch-table.md   # полная таблица диспатча + разрешение имён + коллизии
│       └── toolbox.md          # инвентарь скиллов / MCP / плагинов и агентов
├── code-discovery/          # лестница jetbrains → codebase-memory → grep для поиска кода
│   └── SKILL.md
├── context7-grounding/      # процедура context7-грунтовки перед code-changing диспатчем
│   └── SKILL.md
└── review-loop/             # свежий независимый re-review до чистоты (cap 10)
    ├── SKILL.md
    └── review-loop.md          # Workflow-скрипт + power()-резолвер + схемы
hooks/
├── jetbrains-mcp-probe      # SessionStart-хук: пинг JetBrains MCP → лестница code-discovery
└── settings-snippet.md      # как подключить хук в ~/.claude/settings.json
```

## Установка

Чтобы Claude Code подхватил скиллы, положи каждую директорию из `skills/` в `~/.claude/skills/`.

### macOS / Linux

```bash
git clone https://github.com/ff12s/claude-manager-skill.git
cp -r claude-manager-skill/skills/* ~/.claude/skills/
# хук (опционально): см. hooks/settings-snippet.md
cp claude-manager-skill/hooks/jetbrains-mcp-probe ~/.claude/hooks/ && chmod +x ~/.claude/hooks/jetbrains-mcp-probe
```
...
```powershell
git clone https://github.com/ff12s/claude-manager-skill.git
Copy-Item -Recurse claude-manager-skill\skills\* $env:USERPROFILE\.claude\skills\
# хук (опционально): см. hooks/settings-snippet.md
Copy-Item claude-manager-skill\hooks\jetbrains-mcp-probe $env:USERPROFILE\.claude\hooks\
```
...
Локально каждая живая папка `~/.claude/skills/<skill>` (`manager`, `code-discovery`, `context7-grounding`,
`review-loop`) сделана **directory junction** на соответствующую `skills/<skill>` этого репо
...
обычной установки другими пользователями (они копируют все скиллы целиком, включая `references/`).
